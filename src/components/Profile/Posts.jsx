...
const Posts = ({ photos = null }) => (
  photos != null
      ...
      <>
        {
          photos.length !== 0
            ? (
              <div className="grid grid-cols-1 place-items-center gap-5 sm:grid-cols-2 lg:grid-cols-3 lg:gap-7">
                {
                  photos.map((photo) => {
                    const { photoId } = photo;
                    return (
                      <PostPreview key={photoId} photo={photo} />
                    );
                  })
                }
              </div>
    ...
    : (
      <div className="h-96 grid grid-cols-3 gap-7">
        <Skeleton height="100%" />
        <Skeleton height="100%" />
        <Skeleton height="100%" />
      </div>
